...
def dflines(f, sep="\r?\n", names=["line"], **kw):
    def dfline(f, **kw):
        df = pd.read_csv(f, sep=sep, header=None, names=names, 
                         dtype="object", engine="python", **kw)
        df.insert(0, "recnum", df.index + 1)
        df.insert(0, "filename", f)
        return df
    
    pathes = list(Path(f).lsdir())
    if len(pathes) == 1:
        p = pathes[0]
        return dfline(p, encoding=p.encoding, **kw)
    else:
        return pd.concat(dfline(p, encoding=p.encoding, **kw) for p in pathes)
...
def test():
    ...
    def test_dflines():
        ...
        assert((dflines(f) == pd.DataFrame([[f, 1, "n,aa"], [f, 2, "1,1"], [f, 3, "2,あ"]],
              columns=["filename", "recnum", "line"])).all().all())
