...
util = __import__("util", fromlist = import_module)


def is_target(k, v, mod):
    if k == "main" or any(k.startswith(x) for x in ["_", "test", "dummy"]):
        return False
    return hasattr(v, "__module__") and v.__module__ == mod.__name__

def modlist(mod):
    a = {k:v for k, v in inspect.getmembers(mod) if is_target(k, v, mod)}
    if hasattr(mod, "__all__"):
        a = {x : a[x] for x in set(a) & set(mod.__all__)}
    return a

lazymodules_src = {k: v for mod in import_module for k, v in modlist(getattr(util, mod)).items()}
...
for k, v in lazymodules_src.items():
